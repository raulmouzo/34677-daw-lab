...
<body>
	...
	List<DBProduct> products = conn.getNewProducts();
	%>
	<my-header class="sticky-top"></my-header>
	<main>
		<div class="hero">
			<div class="hero-btn-holder">
				<a href="products.jsp?cat=1" class="btn btn-light hero-btn">Sudaderas</a>
				<a href="products.jsp?cat=2" class="btn btn-light hero-btn">Camisetas</a>
				<a href="products.jsp?cat=3" class="btn btn-light hero-btn">Pantalones</a>
			</div>
			<picture>
		...
		<div class="container">
			<section class="row prd">
				<h1 class="mt-4 mb-3">Nuevos Productos</h1>
						...
						<div class="card">
							<div id="prd-<%= productId %>" class="carousel slide img-prd">
								...
								<div class="sizes">
									...
									<% } else { %>
									<button type="button"
											onclick="addProductToCart(<%= productId %>, <%= variantId %>, '<%= description %>', <%= price %>, '<%= brand %>', 'Nuevos Productos', '<%= size %>', 'img/<%= (images.isEmpty() ? "default.png" : images.get(0)) %>', <%= stock %>)">
											<%= size %>
									</button>
									<% } %>
								<% } %>    
